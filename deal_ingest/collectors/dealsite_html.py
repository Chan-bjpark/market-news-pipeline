"""Dealsite HTML collector. Article URLs: /articles/{id}"""
...
ARTICLE_HREF_RE = re.compile(r"/articles/\d+", re.IGNORECASE)
DATE_RE = re.compile(r"(20\d{2})[.\-/](\d{1,2})[.\-/](\d{1,2})")


class DealsiteHtmlCollector(BaseCollector):
    source = "dealsite"
    source_label = "딜인사이트"
    ...
    def collect(self) -> Iterable[Article]:
        ...
        if not urls:
            self.logger.warning("dealsite_html: no list_urls")
            return
        seen: set[str] = set()
        for list_url in urls:
            self.logger.info("dealsite_html: fetching %s", list_url)
            try:
                resp = self.fetch(list_url)
            except Exception as e:
                self.logger.error("dealsite_html: fetch failed %s: %s",
                                  list_url, e)
                continue
            html = resp.text
            ...
            for a in soup.find_all("a", href=True):
                ...
                count += 1
            self.logger.info("dealsite_html: %d links from %s",
                             count, list_url)
